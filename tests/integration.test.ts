...
function mockProviders() {
  ...
  let i = 0;
  const seed = <T extends { id: string }>(table: string, value: T) => {
    (rows[table] ||= []).push([
      "e" + i++,
      value.id,
      "",
  ...
  global.fetch = async (input, init) => {
    ...
    if (url.includes("api.openai.com")) {
      ...
      assert(b.max_output_tokens <= 8000);
...
test("preparation without Tavily preserves stored sources and warns instead of blocking", async () => {
  const mock = mockProviders();
  try {
    delete process.env.TAVILY_API_KEY;
    const result = await prepare("test", "a1");
    assert(result.warnings.some((w) => w.includes("non configurée")));
    const saved = await new SheetsRepository<Application>(
      "test",
      "Applications",
    ).findById("a1");
    assert(saved?.sources.some((s) => s.url === "https://example.com/atlas"));
    assert.equal(
      (await new SheetsRepository<Document>("test", "Documents").findAll())
        .length,
      3,
    );
  } finally {
