...
export async function fetchPublicPage(url?: string) {
  if (!url) return '';
  try {
    const res = await fetch(url, { headers:{'User-Agent':'Mozilla/5.0 Ubique/1.0'}, next:{revalidate:3600} });
    if (!res.ok) return '';
...
export async function searchWeb(query:string): Promise<SearchResult[]> {
  ...
  const res = await fetch('https://api.tavily.com/search', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({api_key:process.env.TAVILY_API_KEY,query,search_depth:'advanced',max_results:8,include_answer:false}) });
  if (!res.ok) return [];
