...
5. Renseigner `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REDIRECT_URI`, `APP_URL` et `ALLOWED_GOOGLE_EMAIL` (alias `ALLOWED_EMAIL`) dans `.env.local`.
...
`ALLOWED_GOOGLE_EMAIL` (alias `ALLOWED_EMAIL`) est obligatoire : tout autre compte est refusé. La session est chiffrée AES-256-GCM, HttpOnly, SameSite=Lax et Secure en production. State + PKCE protègent OAuth. Les appels d’écriture vérifient l’origine et la session.
...
Renseigner `OPENAI_API_KEY`, `OPENAI_MODEL` et, pour les tâches courtes, `OPENAI_FAST_MODEL` avec un modèle disponible sur le compte API qui prend en charge Responses et le mode JSON. L’abonnement ChatGPT n’inclut pas nécessairement ces usages API.
...
Le dépôt existant est `leojacques-code/Ubique`, branche active `build/v1-application-crm`, PR #1. Le projet Vercel existant est `ubique`, framework Next.js, sorties automatiques. Le runtime est fixé à Node 22.x dans le dépôt. Pousser cette branche construit un preview ; ne pas recréer de dépôt ou de projet.

Utiliser les variables existantes dans Vercel. APP_URL doit correspondre à l’origine réellement ouverte et GOOGLE_REDIRECT_URI à une URI autorisée Google. Le domaine de production peut servir d’origine stable après publication de la PR. Les previews dynamiques ne sont pas ajoutés automatiquement à la liste OAuth.

Ne jamais copier un secret dans le code client ou GitHub. `.gitignore` et `.vercelignore` excluent secrets, CV et dossiers privés.
...
- Companies et Evidence sont réservées à une normalisation future. Les sources complètes de recherche sont archivées dans JobSources, les résumés et preuves restent dans Applications/Profile. Cette simplification est documentée, pas présentée comme un schéma relationnel complet.
...
## Réconciliation Ubique — 16 septembre 2026

La base locale Astra et le commit GitHub `3af857f` sont consolidés. Les exports PDF/DOCX, versions, import CV, synchronisation Gmail paginée/reprenable et transitions protégées sont conservés. Les règles métier, Responses principal/rapide, limites, recherche multi-plateforme, timing et outils de vérification GitHub sont portés.

- Le projet Vercel `ubique` existe ; ses réglages de build sont corrigés. Ne pas recréer le projet pour les anciens échecs `public`.
- Les variables existantes `APP_SESSION_SECRET` (32 caractères minimum), `ALLOWED_GOOGLE_EMAIL` et `GOOGLE_REFRESH_TOKEN` sont acceptées. Le token background n'autorise jamais une session interactive. Les cookies antérieurs nécessitent une nouvelle connexion.
- `/api/auth/google/start` et `/api/auth/google/callback` restent utilisables ; les chemins locaux `/api/auth/login` et `/api/auth/callback` sont conservés.
- Les anciennes lignes Sheets restent intactes et sont lues sous le journal append-only. Les textes des anciens dossiers deviennent des versions historiques dans Documents. Les preuves historiques restent consultables, mais une nouvelle préparation utilise exclusivement le profil validé. Ne pas relancer simultanément l'ancienne version, qui réécrit des onglets entiers.
- Les données privées de `/api/applications` nécessitent désormais une session, comme `/api/snapshot`. La démonstration utilise uniquement des fixtures fictives côté navigateur.
- Sans Tavily, une fiche collée reste préparable ; la recherche externe est signalée absente. Les sources existantes sont conservées.
- Validation : `npm run verify`, `npm audit --audit-level=critical`, puis `npm run smoke -- https://<preview>`. Le smoke accepte un bypass Vercel existant sans le journaliser.

Les documents de passation décrivent des états historiques ; cette section et l'état Git actuel priment. Aucun CV privé ni secret ne figure dans le dépôt.
