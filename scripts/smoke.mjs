...
try{
  ...
  if(!res.ok){
    ...
    if(!type.includes('application/json'))fail('/api/health: expected JSON');
      ...
      else ok(`/api/health: mode=${data.mode||'unknown'} commit=${data.commit||'n/a'}`);
      const serialized=JSON.stringify(data);
      if(/sk-(?:proj-)?[A-Za-z0-9_-]{16,}/.test(serialized))fail('/api/health exposed an API-key-like value');
    }
  }
}catch(error){fail(`/api/health: ${error instanceof Error?error.message:String(error)}`);}
...
for(const path of ['/','/opportunities','/pipeline','/settings']){
  try{
    ...
    if(!text.trim()){fail(`${path}: empty response`);continue;}
    if(!/Ubique|Finance applications|Opportunit|Pipeline|Paramètres/i.test(text))fail(`${path}: expected Ubique page markers not found`);
    else ok(`${path}: HTTP ${res.status}`);
    const robots=(res.headers.get('x-robots-tag')||'').toLowerCase();
    if(!robots.includes('noindex'))fail(`${path}: missing X-Robots-Tag noindex`);
...
try{
  ...
  if(!res.ok)fail(`/api/applications GET: HTTP ${res.status}`);
  else{
    const data=await res.json();
    if(!Array.isArray(data))fail('/api/applications GET: expected array');
    else ok(`/api/applications GET: ${data.length} visible record(s)`);
  }
}catch(error){fail(`/api/applications GET: ${error instanceof Error?error.message:String(error)}`);}

for(const [label,path,body] of [
  ['applications POST','/api/applications',{company:'Smoke Test',jobTitle:'Do not create'}],
  ['action POST','/api/action',{action:'mark-sent',applicationId:'smoke-test'}]
]){
  try{
    const res=await fetchCheck(path,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
    if(res.status!==401)fail(`${label}: expected unauthenticated 401, got ${res.status}`);
...
for(const path of ['/api/cron/watch','/api/cron/gmail-sync']){
  try{
    const res=await fetchCheck(path);
    if(res.status!==401)fail(`${path}: expected 401 without CRON_SECRET, got ${res.status}`);
